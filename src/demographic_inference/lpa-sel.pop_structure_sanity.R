...
gt_df <- get_gt_df_from_raw("data/demographic_inference/lpa-sel.masked_regions_only.raw")
...
# list of sel individuals and sel only df
sel_samples <- rownames(all_df)[grep("ll", rownames(all_df))]
sel_df <- all_df[rownames(all_df) %in% sel_samples, ]
# filter out non-variants
different_columns <- sapply(sel_df, function(x) length(unique(x)) > 1)
sel_df <- sel_df[, different_columns]
...
ggsave(filename = paste0("plots/demographic_inference/sanity/lpa-sel.all_pca.pdf"),
...
ggsave(filename = paste0("plots/demographic_inference/sanity/lpa-sel.lpa_pca.pdf"),
       plot = lpa_pca_plot, width = 300, height = 160, units = "mm")

# sel samples - pca
sel_pca <- FactoMineR::PCA(sel_df, graph = F)
# sel samples - pca dataframe
sel_pca_df <- build_df_from_pca(pca_obj = sel_pca)
# sel samples - variance percents
sel_pca_variance_percents <- sel_pca$eig[,2]
# sel samples - pca plot
sel_pca_plot <- plot_pca(pca_df = sel_pca_df,
                         variance_percents = sel_pca_variance_percents)

ggsave(filename = paste0("plots/demographic_inference/sanity/lpa-sel.sel_pca.pdf"),
       plot = sel_pca_plot, width = 300, height = 160, units = "mm")
