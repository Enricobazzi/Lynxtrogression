...
gt_df <- get_gt_df_from_raw("data/demographic_inference/lpa-wel.masked_regions_only.raw")
...
# list of wel individuals and wel only df
wel_samples <- rownames(all_df)[grep("ll", rownames(all_df))]
wel_df <- all_df[rownames(all_df) %in% wel_samples, ]
# filter out non-variants
different_columns <- sapply(wel_df, function(x) length(unique(x)) > 1)
wel_df <- wel_df[, different_columns]
...
ggsave(filename = paste0("plots/demographic_inference/sanity/lpa-wel.all_pca.pdf"),
...
ggsave(filename = paste0("plots/demographic_inference/sanity/lpa-wel.lpa_pca.pdf"),
       plot = lpa_pca_plot, width = 300, height = 160, units = "mm")

# wel samples - pca
wel_pca <- FactoMineR::PCA(wel_df, graph = F)
# wel samples - pca dataframe
wel_pca_df <- build_df_from_pca(pca_obj = wel_pca)
# wel samples - variance percents
wel_pca_variance_percents <- wel_pca$eig[,2]
# wel samples - pca plot
wel_pca_plot <- plot_pca(pca_df = wel_pca_df,
                         variance_percents = wel_pca_variance_percents)

ggsave(filename = paste0("plots/demographic_inference/sanity/lpa-wel.wel_pca.pdf"),
       plot = wel_pca_plot, width = 300, height = 160, units = "mm")
